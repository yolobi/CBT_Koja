...
def start():
    ...
    if(auth['bidang'] == 'matematika' or auth['bidang'] == 'biologi' or auth['bidang'] == 'komputer'):
    ...
    if request.method == 'POST':
        ...
        if auth['bidang'] == 'biologi':
            ...
            for i in range(len(data)//4):
                ...
                c = (data['c'+str(i+1)])
                d = (data['d'+str(i+1)])
                cur.execute("INSERT INTO history (uid, answer, submitted_at) VALUES (%s, %s, %s)", (auth['uid'], a, date))
                mysql.commit()
                cur.execute("INSERT INTO history (uid, answer, submitted_at) VALUES (%s, %s, %s)", (auth['uid'], b, date))
                mysql.commit()
                cur.execute("INSERT INTO history (uid, answer, submitted_at) VALUES (%s, %s, %s)", (auth['uid'], c, date))
                mysql.commit()
                cur.execute("INSERT INTO history (uid, answer, submitted_at) VALUES (%s, %s, %s)", (auth['uid'], d, date))
                mysql.commit()

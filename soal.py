    ...
    if (request.method == 'GET' and request.cookies.get("auth") and request.cookies.get("session")):
        ...
        if auth['bidang'] == 'komputer':
            ...
            if res['kj'] == 'essay' or res['kj'] == 'essai':
                return render_template("soal_essai.html", user=auth, bidang=res, len=total, timer=timer['exp'])
            elif res['opsi_A'] == res['opsi_B'] == res['opsi_C'] == res['opsi_D'] == res['opsi_E'] == '':
                return render_template("isian_singkat.html", user=auth, bidang=res, len=total, timer=timer['exp'])
            else:
                return render_template("pilihan_ganda.html", user=auth, bidang=res, len=total, timer=timer['exp'])
        else:
